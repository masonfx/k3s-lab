...
EOF
    ...
    cat <<EOF > /etc/sysctl.d/85-network-optimizations.conf
...
EOF
    ...
    cat <<EOF > /etc/sysctl.d/85-memory-optimizations.conf
...
EOF
    ...
    cat <<EOF > /etc/sysctl.d/85-kernel-optimizations.conf
...
function harden_ssh() {
    ...
    su mfx -c 'mkdir -p $HOME/.ssh && google-authenticator -t -d -f -r 3 -R 30 -w 3 -Q UTF8 -i "$(echo $(hostname --fqdn))" -l "mfx@$(echo $(hostname --fqdn))" && mv $HOME/.google_authenticator $HOME/.ssh/google_authenticator && chmod 400 $HOME/.ssh/google_authenticator && chown mfx:mfx $HOME/.ssh/google_authenticator'
...
EOF
    ...
    cat <<EOF > /etc/ssh/sshd_config.d/00-os-hardening.conf
